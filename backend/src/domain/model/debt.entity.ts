...
export class Debt extends BaseEntity {
  ...
  constructor(
    ...
    this.validateBalances(originalAmount, currentBalance, debtType);
   ...
   */
  private validateBalances(originalAmount: Money, currentBalance: Money, debtType: DebtType): void {
    if (debtType === 'mortgage' && currentBalance.cents > originalAmount.cents) {
      throw new ValidationError('Mortgage balance cannot exceed original debt amount');
    }
