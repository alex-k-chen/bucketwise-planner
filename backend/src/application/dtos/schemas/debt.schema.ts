...
export const createDebtSchema = z
  ...
  .superRefine((val, ctx) => {
    if (val.debtType === 'mortgage' && val.currentBalanceCents > val.originalAmountCents) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'Mortgage balance cannot exceed original amount',
    ...
    if (val.debtType === 'mortgage' && val.interestRate > 0.1) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
